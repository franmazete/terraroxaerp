...
interface Props {
  ...
  dadosSSR?: OrdemDetalheSSR | null;
}

export function OrdemDetalheClientView({ ocId, dadosSSR = null }: Props) {
  const id = ocId;
  ...
  // Resolve OC e entidades vinculadas — prefere SSR quando disponível
  const oc = usandoSSR ? dadosSSR!.oc : ordens.find((o) => o.id === id);
  if (!oc) notFound();

  // Restringe transportadora a ver só as próprias OCs
  if (user?.role === "transportadora" && oc.transp_id !== user.transp_id) notFound();

  const contrato = usandoSSR ? dadosSSR!.contrato : contratos.find((c) => c.id === oc.contrato_id) ?? null;
  const carga = usandoSSR ? dadosSSR!.carga : cargas.find((c) => c.id === oc.carga_id) ?? null;
  const transp = usandoSSR ? dadosSSR!.transp : transportadoras.find((t) => t.id === oc.transp_id) ?? null;
  const motorista = usandoSSR ? dadosSSR!.motorista : motoristas.find((m) => m.id === oc.motorista_id) ?? null;
  const veiculo = usandoSSR ? dadosSSR!.veiculo : veiculos.find((v) => v.id === oc.veiculo_id) ?? null;
  const origem = usandoSSR ? dadosSSR!.origem : locais.find((l) => l.id === oc.local_carg_id) ?? null;
  const destino = usandoSSR ? dadosSSR!.destino : (locais.find((l) => l.id === oc.destino_local_id) ?? null);
  const terminal = usandoSSR ? dadosSSR!.terminal : (oc.terminal_id ? (terminais.find((t) => t.id === oc.terminal_id) ?? null) : null);
  const produto = usandoSSR ? dadosSSR!.produto : (contrato ? (produtos.find((p) => p.id === contrato.produto_id) ?? null) : null);
  const produtor = usandoSSR ? dadosSSR!.produtor : (contrato ? (produtores.find((p) => p.id === contrato.produtor_id) ?? null) : null);
  ...
  return (
    <>
      ...
      {/* Card destacado: Carregamento Autorizado (Ordem interna Terra Roxa) */}
      <AutorizacaoCarregamentoCard
        oc={oc}
        produtoNome={produto?.nome ?? carga?.produto ?? null}
        transp={transp ?? null}
        motorista={motorista ?? null}
        veiculo={veiculo ?? null}
        origem={origem ?? null}
